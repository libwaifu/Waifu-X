...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated Waifu loader file *)


Unprotect[Waifu`PackageScope`Private`$TopLevelLoad];
Waifu`PackageScope`Private`$TopLevelLoad = MemberQ[$ContextPath, "Global`"];
...
ClearAll[Waifu];
Waifu::usage = "Waifu is a manager head for the Waifu package";
...
Waifu["LoadingParameters"] := $PackageLoadSpecs
$PackageLoadSpecs =
	Merge[
		{
			With[
...
Waifu["PackageListing"] := $PackageListing;
$PackageListing = <||>;
Waifu["Contexts"] := $PackageContexts;
If[!ListQ@$PackageContexts,
	$PackageContexts =
		{
			"Waifu`",
			"Waifu`PackageScope`Private`",
			"Waifu`PackageScope`Package`"
		}
];
$PackageDeclared =
...
Waifu["FEScopedSymbols"] := $PackageFEHiddenSymbols;
$PackageFEHiddenSymbols = {};
Waifu["PackageScopedSymbols"] := $PackageScopedSymbols;
...
Waifu["FileContexts"] := $PackageFileContexts;
...
Waifu["DeclaredPackages"] := $DeclaredPackages;
...
Waifu["LoadedPackages"] := $LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ] :=
	With[{s = PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s, Waifu], "`"]
	];
...
Waifu::nodep = "Couldn't load dependency `` of type ``";
Waifu::nodup = "Couldn't update dependency `` of type ``";
...
PackageInstallPackageDependency[dep_String, ops : OptionsPattern[]] :=
	Block[{retcode, site, path, file, tmp},
		...
		If[retcode < 300,
			...
			DeleteFile[tmp];
			file,
			Message[Waifu::nodep, dep, "Package"];
			DeleteFile[tmp];
			$Failed
		]
	];
...
PackageCatchMessage[
	...
	Replace[
		Thread[Map[Hold[MessageName[Waifu, #]]&, msg], Hold],
		{
			{} :>
				Check[expr,
...
PackageFailureException[
	...
	(
		Set[MessageName[Waifu, tag], body];
		PackageFailureException[
			MessageName[Waifu, tag],
			body,
			ops
...
PackageAttachMainAutocomplete[] :=
	PackageAddAutocompletions[Waifu,
		Table[
			Replace[{} -> None]@
				Cases[
					DownValues[Waifu],
...
If[
	(Clear@Waifu`PackageScope`Private`$loadAbort;!#)&@
		Waifu`PackageScope`Private`$loadAbort,
	Unprotect[Waifu`PackageScope`Private`$LoadCompleted];
	Waifu`PackageScope`Private`$LoadCompleted = True;
	Waifu`PackageScope`Private`PackagePostProcessContextPathReassign[]
]