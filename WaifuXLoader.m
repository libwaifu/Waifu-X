...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated WaifuX loader file *)


Unprotect[WaifuX`PackageScope`Private`$TopLevelLoad];
WaifuX`PackageScope`Private`$TopLevelLoad = MemberQ[$ContextPath, "Global`"];
...
ClearAll[WaifuX];
WaifuX::usage = "WaifuX is a manager head for the WaifuX package";
...
WaifuX["LoadingParameters"] := $PackageLoadSpecs
$PackageLoadSpecs =
	Merge[
		{
			With[
...
WaifuX["PackageListing"] := $PackageListing;
$PackageListing = <||>;
WaifuX["Contexts"] := $PackageContexts;
If[!ListQ@$PackageContexts,
	$PackageContexts =
		{
			"WaifuX`",
			"WaifuX`PackageScope`Private`",
			"WaifuX`PackageScope`Package`"
		}
];
$PackageDeclared =
...
WaifuX["FEScopedSymbols"] := $PackageFEHiddenSymbols;
$PackageFEHiddenSymbols = {};
WaifuX["PackageScopedSymbols"] := $PackageScopedSymbols;
...
WaifuX["FileContexts"] := $PackageFileContexts;
...
WaifuX["DeclaredPackages"] := $DeclaredPackages;
...
WaifuX["LoadedPackages"] := $LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ] :=
	With[{s = PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s, WaifuX], "`"]
	];
...
WaifuX::nodep = "Couldn't load dependency `` of type ``";
WaifuX::nodup = "Couldn't update dependency `` of type ``";
...
PackageInstallPackageDependency[dep_String, ops : OptionsPattern[]] :=
	Block[{retcode, site, path, file, tmp},
		...
		If[retcode < 300,
			...
			DeleteFile[tmp];
			file,
			Message[WaifuX::nodep, dep, "Package"];
			DeleteFile[tmp];
			$Failed
		]
	];
...
PackageCatchMessage[
	...
	Replace[
		Thread[Map[Hold[MessageName[WaifuX, #]]&, msg], Hold],
		{
			{} :>
				Check[expr,
...
PackageFailureException[
	...
	(
		Set[MessageName[WaifuX, tag], body];
		PackageFailureException[
			MessageName[WaifuX, tag],
			body,
			ops
...
PackageAttachMainAutocomplete[] :=
	PackageAddAutocompletions[WaifuX,
		Table[
			Replace[{} -> None]@
				Cases[
					DownValues[WaifuX],
...
If[
	(Clear@WaifuX`PackageScope`Private`$loadAbort;!#)&@
		WaifuX`PackageScope`Private`$loadAbort,
	Unprotect[WaifuX`PackageScope`Private`$LoadCompleted];
	WaifuX`PackageScope`Private`$LoadCompleted = True;
	WaifuX`PackageScope`Private`PackagePostProcessContextPathReassign[]
]